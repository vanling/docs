...
each operation executes, it has access to the Flow Object. Once an operation finishes, its data is appended under
`<operationKey>`. If the operation didn't generate data, `null` is appended as the value under its key.
...
```json
{
	"$trigger": {
		...
		// Typically includes "header" data from the request to the trigger.
...
lot of data. This data was intentionally left out for the example because the exact data nested in these first-level
keys are always going to be slightly different, based on your flow's unique configuration. During configuration, you'll
need to use a tool like the [Logger](#logs) to view the data you are working with.

:::tip

In our examples, we are using generic names for operation keys, like `<operationKey>`. Low-code users might not be aware
that this is a _placeholder_, used for the sake of example. In practice, operation keys will have a unique and
descriptive names, like `send_email_7538`.
...
<!--
Add a video to demo this within the app itself.
"{{}}" versus {{}}
-->

While configuring an operation, Directus lets use any key from the Flow Object as a variable. You simply wrap the
variable in double-moustache syntax.
